//Compilação: gcc AggresiveSquares.c -o AggresiveSquares $(pkg-config allegro-5 allegro_main-5 allegro_font-5 --libs --cflags)
	...
	while(1){																		//Laço principal do programa
		...
		if (event.type == 30){														//O evento tipo 30 indica um evento de relógio, ou seja, verificação se a tela deve ser atualizada (conceito de FPS)
			if(flag < 5){
				 ...
				 flag++;							//Substitui tudo que estava desenhado na tela por um fundo preto
			}else{
